...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { firstName, email, phone, quizData } = req.body;
    ...
    const customFields = formatCustomFields(quizData);
 ...
 */
function formatCustomFields(quizData) {
  const fields = [];
  ...
  // Main goals (array to comma-separated string)
  if (quizData.step6) {
    const goals = Array.isArray(quizData.step6) ?
      quizData.step6.join(', ') :
      quizData.step6;
    fields.push({
      key: 'main_goals',
